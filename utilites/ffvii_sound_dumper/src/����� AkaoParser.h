...
public:
	...
	// voice routines
	int LoadVoiceData(const char *arg_all_file, const char *arg_dat_file);
	...
	{
		T result = *(T *)&m_ChannelData[arg_ch].stream[m_ChannelData[arg_ch].current_pos];
		m_ChannelData[arg_ch].current_pos += sizeof(result);
		return result;
	}

	void Update(float arg_delta);
...
private:
    ...
    // voice
    std::vector< VoiceIndex > m_VoiceIndex;

    // sequence variables
    AkaoHeader m_SequenceHeader;
    std::string m_SequenceDataFilename;
    AkaoChannelData m_ChannelData[ m_AKAO_CHANNELS_COUNT ];
    u16 m_Tempo;
    float m_MSPassed;
    float m_MSPeriodicalPassed;
	...
	// processing
	void PeriodicalProcess(float arg_delta);
	void OpcodeProcess(float arg_delta);

	// HAL
	void SpuSetParameter(SpuConfig &arg_cfg, SpuParameter arg_param, u32 arg_value);
	void SpuUpdate();

	// too big prototype list, separated
#define AKAO_PARSER_INSIDE
	#include "AkaoParserOpcodes.h"
#undef AKAO_PARSER_INSIDE
	...
	// FIXME!!
	std::vector<u32> m_PsxAttackOffset;
	std::vector<u32> m_PsxLoopOffset;
};
