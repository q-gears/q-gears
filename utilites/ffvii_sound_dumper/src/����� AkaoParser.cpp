...
AkaoParser::AkaoParser():
    ...
    m_IsPlaying(false)
{
    memset( (u8*)m_ChannelData, 0x00, sizeof( AkaoChannelData ) * m_AKAO_CHANNELS_COUNT );
}
...
int AkaoParser::LoadVoiceData(const char *arg_all_file, const char *arg_dat_file)
{
	/* dma transfer [sample data from INSTR.ALL] */
	// temporary writing dma dump gotten with real emu
	{
		std::ifstream iF;
		iF.open(arg_all_file, std::ios_base::binary);
		uint32_t datasize = 483264;
		u8* buffer = new u8[datasize];
		iF.read((char *)buffer, datasize);
		iF.close();
		if(!SOUNDMAN->PsxWriteDMA(0x0202, buffer, datasize))
		{
			std::cout << "[SoundManager] error: failed to DMA write sample data" << std::endl;
			exit(EXIT_FAILURE);
		}
		delete [] buffer;
	}
	...
	// open index file
	std::ifstream iIndexF;
	iIndexF.open(arg_dat_file, std::ios_base::binary);
	if(!iIndexF.is_open())
	{
		std::cout << "[AkaoParser] error: cann't open " << arg_dat_file << std::endl;
		return 1;
	}
	...
	while(true)
	{
		iIndexF.read((char *)&voice_index_record, sizeof(voice_index_record));
		...
		voice_index_record.loop_offset -= (m_AKAO_INSTR_ALL_SHIFT + voice_index_record.attack_offset);
...
int AkaoParser::LoadSequenceData(const char *arg_file)
{
	...
	std::ifstream iF;
	iF.open(arg_file, std::ios_base::binary | std::ios_base::in);
	if(!iF.is_open())
		return 1;
	iF.read((char *)&m_SequenceHeader, sizeof(m_SequenceHeader));
	if(memcmp(m_AKAO_MAGIC, m_SequenceHeader.magic, sizeof(m_SequenceHeader.magic)))
    {
        return 2;
    }
	...
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
	{
		m_ChannelData[i].stream_length = 0;
		if(channels_info & (1 << i))
		{
			m_ChannelData[i].stream_length = buffer_offsets[buffer_index + 1] - buffer_offsets[buffer_index];

			m_ChannelData[i].stream = new u8[m_ChannelData[i].stream_length];

			memcpy((char *)m_ChannelData[i].stream, (char *)&buffer[buffer_offsets[buffer_index]], m_ChannelData[i].stream_length);
	...
	m_SequenceDataFilename = std::string(arg_file);
...
void AkaoParser::UnloadSequenceData()
{
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
	{
		if(m_ChannelData[i].stream)
			delete [] m_ChannelData[i].stream;
	}
...
int AkaoParser::PlaySequenceData()
{
	...
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
	{
		if(m_ChannelData[i].stream == NULL)
			continue;

		// voice
		m_ChannelData[i].voice_index = 0x14;
		// (at least it sets to 0x14 by default in Square code)

		// position
		m_ChannelData[i].current_pos = 0;
		m_ChannelData[i].saved_pos_index = 0;
		for(int j = 0; j < 4; ++j)
		{
			m_ChannelData[i].saved_pos[j] = 0;
			m_ChannelData[i].saved_unknown[j] = 0;
		}

		// pause
		m_ChannelData[i].pause = 0;
		m_ChannelData[i].pause_index = 0;
		m_ChannelData[i].pause_multiplier = 0;

		// pitch
		m_ChannelData[i].pitch_base = 0;
		m_ChannelData[i].pitch_unknown1 = 0;
		m_ChannelData[i].pitch_80062fe6 = 0;
		m_ChannelData[i].pitch_correction = 0;
		m_ChannelData[i].pitch_selector = 0;
		m_ChannelData[i].pitch_modifier = 0;
		m_ChannelData[i].pitch_saved_parameters = 0;
		m_ChannelData[i].pitch_wave_shift = 0;

		// volume
		m_ChannelData[i].volume_multiplier = m_AKAO_DEFAULT_VOLUME_MODIFIER;
		m_ChannelData[i].volume_level = 0;
		m_ChannelData[i].volume_level_changed = false;
		m_ChannelData[i].volume_index = 0;
		m_ChannelData[i].volume_index_changed = false;
		SpuSetParameter(m_ChannelData[i].spu_config, SPU_VOLUME_LEFT, 0x3fff);
		SpuSetParameter(m_ChannelData[i].spu_config, SPU_VOLUME_RIGHT, 0x3fff);

		// wave
		m_ChannelData[i].wave_delay = 0;
		m_ChannelData[i].wave_delay_current = 0;
		m_ChannelData[i].wave_refresh_interval = 0;
		m_ChannelData[i].wave_refresh_interval_current = 0;
		m_ChannelData[i].wave_table_node_index = 0;
		m_ChannelData[i].wave_multiplier = 0;
		m_ChannelData[i].wave_modifier = 0;
		m_ChannelData[i].wave_table_index = 0;
		m_ChannelData[i].wave_loaded = false;

		m_ChannelData[i].mirror = 0;
		m_ChannelData[i].mirror_enabled = false;
...
void AkaoParser::Update(float arg_delta)
{
...
void AkaoParser::PeriodicalProcess(float arg_delta)
{
	...
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
	{
		...
		if(m_ChannelData[i].wave_delay_current)
			--m_ChannelData[i].wave_delay_current;

		if(!m_ChannelData[i].wave_delay_current &&
				m_ChannelData[i].wave_loaded)
		{
			--m_ChannelData[i].wave_refresh_interval_current;
			if(!m_ChannelData[i].wave_refresh_interval_current)
			{
				// ch[0x76] - here is 2nd parameter of B4
				m_ChannelData[i].wave_refresh_interval_current =
					m_ChannelData[i].wave_refresh_interval;
				...
				if(!m_AKAO_WAVE_TABLE[m_ChannelData[i].wave_table_index] &&
					!m_AKAO_WAVE_TABLE[m_ChannelData[i].wave_table_index + 1])
				{
					m_ChannelData[i].wave_table_index +=
						m_AKAO_WAVE_TABLE[m_ChannelData[i].wave_table_index + 2];
				}

				s16 a3;
				{
					s32 v0 = m_ChannelData[i].wave_multiplier *
						m_AKAO_WAVE_TABLE[m_ChannelData[i].wave_table_index];
				...
				if(a3 != m_ChannelData[i].pitch_wave_shift)
				{
					m_ChannelData[i].pitch_wave_shift = a3;

					if(m_ChannelData[i].pitch_wave_shift >= 0)
						m_ChannelData[i].pitch_wave_shift *= 2;
		...
		if(pitch_changed)
		{
			...
			u16 current_pitch = m_ChannelData[i].pitch_base +
				m_ChannelData[i].pitch_unknown1 +
				m_ChannelData[i].pitch_wave_shift;

			if(m_ChannelData[i].pitch_80062fe6)
			{
				std::cout << "*** WARNING: can't show this message for now!" << std::endl;

				s32 v0 = current_pitch * m_ChannelData[i].pitch_80062fe6;
				if(m_ChannelData[i].pitch_80062fe6 < 0x80)
				{
					// sra
					current_pitch += v0 >> 7;
				}
			...
			SpuSetParameter(m_ChannelData[i].spu_config, SPU_PITCH, current_pitch);
		}
	}
}


void AkaoParser::OpcodeProcess(float arg_delta)
{
	// cumulate passed milliseconds
	...
	{
		// pause calculation
		--m_ChannelData[i].pause;

		// here is non-blocking periodical processing
		{
			// processing for opcode A9 (volume increase/decrease)
			if(m_ChannelData[i].volume_level_change_ticks)
			{
				--m_ChannelData[i].volume_level_change_ticks;
				if(((m_ChannelData[i].volume_level +
						m_ChannelData[i].volume_level_new) & 0xFFE00000) !=
						(m_ChannelData[i].volume_level & 0xFFE00000))
					m_ChannelData[i].volume_level_changed = true;

				m_ChannelData[i].volume_level +=
					m_ChannelData[i].volume_level_new;
			}

			// volume setting, ugly
			// directly taken from FF7 binary :)
			if(m_ChannelData[i].volume_level_changed || m_ChannelData[i].volume_index_changed)
			{
				int32_t temp = (unsigned)((m_ChannelData[i].volume_level >> 16) * m_ChannelData[i].volume_multiplier) >> 7;
				temp *= m_AKAO_DEFAULT_VOLUME_MODIFIER;
				temp >>= 7;

				int32_t volume_left = (temp * m_AKAO_VOLUME_TABLE_L[ m_ChannelData[i].volume_index]) >> 15;
				int32_t volume_right = (temp * m_AKAO_VOLUME_TABLE_R[ m_ChannelData[i].volume_index]) >> 15;

				SpuSetParameter(m_ChannelData[i].spu_config, SPU_VOLUME_LEFT, volume_left);
				SpuSetParameter(m_ChannelData[i].spu_config, SPU_VOLUME_RIGHT, volume_right);
		...
		if(m_ChannelData[i].pause || m_ChannelData[i].stream == NULL)
        {
			continue;
        }
	...
	do
	{
		...
		else
		{
			...
			PrintOpcode(i, current_opcode);
			std::cout << std::endl;
			m_ChannelData[i].current_pos += m_OpcodesParametersCount[opcode_index];
		}
	...
	if(current_opcode == 0xa0)
		...
		// index file and silence from table
		{
			...
			// custom pause
			if(m_ChannelData[i].pause_multiplier)
			{
				m_ChannelData[i].pause = m_ChannelData[i].pause_multiplier;
			}
			else
			{
				m_ChannelData[i].pause = m_AKAO_PAUSE_TABLE[pause_index];
			}

			m_ChannelData[i].pause_index = m_ChannelData[i].pause;
		}
		...
		if(current_opcode < 0x84)
		{
			...
			if(pitch_index < 12)
			{
				...
				m_ChannelData[i].pitch_saved_parameters =
					pitch_index + m_ChannelData[i].pitch_correction * 12;

				// pitch selector
				pitch_index += m_ChannelData[i].pitch_selector;

				// load selected pitch from table
				u32 current_pitch = m_VoiceIndex[
					m_ChannelData[i].voice_index].pitch[pitch_index];

				// apply pitch correction
				if(m_ChannelData[i].pitch_correction >= 7)
					current_pitch <<= (m_ChannelData[i].pitch_correction - 6);
				else if(m_ChannelData[i].pitch_correction < 6)
					current_pitch >>= (6 - m_ChannelData[i].pitch_correction);

				// pitch modifier
				if(m_ChannelData[i].pitch_modifier)
				{
					current_pitch +=
						current_pitch * m_ChannelData[i].pitch_modifier /
						(m_ChannelData[i].pitch_modifier > 0 ? 128 : 256);

					// make sure we are not out of 0xffff range
					current_pitch = current_pitch & 0xffff;
				}

				m_ChannelData[i].pitch_base = current_pitch;
...
void AkaoParser::SpuSetParameter(SpuConfig &arg_cfg, SpuParameter arg_param, u32 arg_value)
{
	switch(arg_param)
	{
...
void AkaoParser::SpuUpdate()
{
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
	{
		SpuConfig &config = m_ChannelData[i].spu_config;
...
void AkaoParser::LoadVoice(int arg_ch, int arg_voice)
{
	...
	m_ChannelData[arg_ch].voice_index = arg_voice;

	// FIXME!!
//	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_ADDRESS_START,
//			m_VoiceIndex[arg_voice].attack_offset);
//	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_ADDRESS_LOOP,
//			m_VoiceIndex[arg_voice].loop_offset);
	...
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_ATTACK_RATE,
			m_VoiceIndex[arg_voice].attack_rate);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_ATTACK_MODE,
			m_VoiceIndex[arg_voice].attack_mode);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_SUSTAIN_RATE,
			m_VoiceIndex[arg_voice].sustain_rate);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_SUSTAIN_MODE,
			m_VoiceIndex[arg_voice].sustain_mode);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_RELEASE_RATE,
			m_VoiceIndex[arg_voice].release_rate);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_RELEASE_MODE,
			m_VoiceIndex[arg_voice].release_mode);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_DECAY_RATE,
			m_VoiceIndex[arg_voice].decay_rate);
	SpuSetParameter(m_ChannelData[arg_ch].spu_config, SPU_SUSTAIN_LEVEL,
...
void AkaoParser::RefreshWaveParameters(int arg_ch)
{
	...
	if(m_ChannelData[arg_ch].wave_modifier & 0x8000)
	{
		temp = ((m_ChannelData[arg_ch].wave_modifier & 0x7f00) >> 8) *
			m_ChannelData[arg_ch].pitch_base;
	}
	else
	{
		temp = ((m_ChannelData[arg_ch].wave_modifier & 0x7f00) >> 8) *
			(((int)(15 * m_ChannelData[arg_ch].pitch_base)) >> 8);
	}

	m_ChannelData[arg_ch].wave_multiplier = temp >> 7;

	//	m_ChannelData[i].wave_multiplier =
	//		(s16)((s32)(((m_ChannelData[i].wave_modifier & 0x7f00) >> 8) *
	//		(m_ChannelData[i].wave_modifier & 0x8000 ?
	//		m_ChannelData[i].pitch_base :
	//		((s32)(15 * m_ChannelData[i].pitch_base)) >> 8)) >> 7);
}
...
void AkaoParser::PrintOpcode(int arg_ch, u8 arg_opc)
{
	...
	if(parameters_count)
    {
        ...
        {
			std::cout << ' ' << std::setw(2) << (int)m_ChannelData[arg_ch].stream[m_ChannelData[arg_ch].current_pos + i];
        }
    }
...
int AkaoParser::DumpSequenceData(const char *arg_file)
{
	...
	std::ofstream oF;
	oF.open(arg_file, std::ios_base::binary | std::ios_base::out);
	if(!oF.is_open())
		return 1;

	oF << std::setfill('0');
	oF << std::hex;
	oF << "*** Frame ID: 0x" << std::setw(4) << m_SequenceHeader.id << std::endl;
	oF << std::dec;
	oF << "*** Frame Length: " << m_SequenceHeader.length << " bytes" << std::endl;
	oF << std::hex;
	oF << "*** Reverb Mode: 0x" << std::setw(4) << m_SequenceHeader.reverb_mode << std::endl;
	oF << std::hex;
	oF << "*** Unused Data: ";
	for(int i = 0; i < 6; ++i)
    {
		oF << std::setw(2) << (int)m_SequenceHeader.unused[i] << ' ';
    }
	oF << std::endl;
	...
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
		if(m_ChannelData[i].stream)
			++channels_count;
	oF << std::dec;
	oF << "*** Channels count: " <<std:: setw(2) << (int)channels_count << std::endl << std::endl;
	...
	for(int i = 0; i < m_AKAO_CHANNELS_COUNT; ++i)
	{
		...
		oF << std::hex;
		u16 j = 0;
		while(j < m_ChannelData[i].stream_length)
		{
			switch(m_ChannelData[i].stream[j])
			{
				// no parameters
				case 0xa0:
				case 0xa6:
				...
				case 0xfc:
				case 0xff:
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j];
					oF << std::endl;
					++j;
					break;
				...
				case 0xf8:
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j] << ' ';
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j + 1];
					oF << std::endl;
					j += 2;
					break;
				...
				case 0xfe:
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j] << ' ';
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j + 1] << ' ';
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j + 2];
					oF << std::endl;
					j += 3;
					break;
				...
				case 0xf1:
					...
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j + 1] << ' ';
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j + 2] << ' ';
					oF << std::setw(2) << (int)m_ChannelData[i].stream[j + 3];
					oF << std::endl;
					j += 4;
					break;
				...
				default:
					oF << std::setw(2)
						<< (int)m_ChannelData[i].stream[j];
					if(j < m_ChannelData[i].stream_length - 1 &&
							m_ChannelData[i].stream[j + 1] < 0xa0)
						oF << ' ';
					else
						oF << std::endl;
					++j;
