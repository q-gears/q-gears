...
SoundManager::SoundManager(uint16_t arg_frequency, uint8_t arg_output_channels,
		uint32_t arg_buffer_size, uint8_t arg_input_channels,
		char *arg_psx_plugin_path):
	m_Started(true)
{
	uint8_t *buffer;
/*
	// open plugin library
	m_PluginHandle = dlopen(arg_psx_plugin_path, RTLD_LAZY);
	if(m_PluginHandle == NULL)
	{
		cout << "[Sound Manager] error: failed to load plugin library."
		   << dlerror() << endl;
		exit(EXIT_FAILURE);
	}

	SPUinit =
		(SPUinit_t)dlsym(m_PluginHandle, "SPUinit");
	SPUopen =
		(SPUopen_t)dlsym(m_PluginHandle, "SPUopen");
	SPUclose =
		(SPUclose_t)dlsym(m_PluginHandle, "SPUclose");
	SPUasync =
		(SPUasync_t)dlsym(m_PluginHandle, "SPUasync");
	SPUreadRegister =
		(SPUreadRegister_t)dlsym(m_PluginHandle, "SPUreadRegister");
	SPUwriteRegister =
		(SPUwriteRegister_t)dlsym(m_PluginHandle, "SPUwriteRegister");
	SPUwriteDMAMem =
		(SPUwriteDMAMem_t)dlsym(m_PluginHandle, "SPUwriteDMAMem");
*/
	...
	{
		ifstream iF;
		iF.open("data/dmadump.bin", ios_base::binary);
		...
		{
			cout << "[SoundManager] error: failed to DMA write sample data"
				<< endl;
			exit(EXIT_FAILURE);
		}
		delete [] buffer;
	...
	{
		ifstream iF;
		iF.open("data/dmadump2.bin", ios_base::binary);
		...
		{
			cout << "[SoundManager] error: failed to DMA write unknown data"
				<< endl;
			exit(EXIT_FAILURE);
		}
		delete [] buffer;
	...
	/* and finishing dma transfer of zeroes :), phew */
	{
		...
		for(int i = 0; i < 28; ++i)
		{
			...
			{
				cout << "[SoundManager] error: failed to DMA write 0x00 data"
					<< endl;
				exit(EXIT_FAILURE);
			}
		}
...
bool SoundManager::PsxWriteDMA(uint16_t arg_address, uint8_t *arg_data,
		uint32_t arg_length)
{
	uint16_t temp;
...
void SoundManager::StartProcessing()
{
	// don't needed now
	m_Started = true;
}
...
void SoundManager::StopProcessing()
{
	// don't needed now
	m_Started = false;
}
...
void SoundManager::Update(uint32_t arg_ms)
{
	if(m_Started)
	{
		SPUasync(0xc000);
...
void SoundManager::PsxInstrumentSetReverbDepth(uint8_t arg_channel,
	uint16_t arg_left, uint16_t arg_right)
{
...
void SoundManager::PsxChannelPlay(uint8_t arg_channel)
{
	if(arg_channel < 16)
		// H_SPUon1
	...
	{
		cout << "[SoundManager] error: unknown channel ON" << endl;
		exit(EXIT_FAILURE);
	}
}


void SoundManager::PsxReverbOn(uint8_t arg_channel)
{
	if(arg_channel < 16)
		// H_RVBon1
	...
	{
		cout << "[SoundManager] error: unknown channel REVERB" << endl;
		exit(EXIT_FAILURE);
	}
}


void SoundManager::PsxChannelStop(uint8_t arg_channel)
{
	if(arg_channel < 16)
		// H_SPUoff1
	...
	{
		cout << "[SoundManager] error: unknown channel OFF" << endl;
		exit(EXIT_FAILURE);
	}
}
...
void SoundManager::SetVoiceStartAddress(u8 arg_voice, u32 arg_address)
{
	...
	SPUwriteRegister(m_PSX_SPU_BASE + arg_voice * 0x10 + 0x06,
			(u16)(arg_address / 8));
}
...
void SoundManager::SetVoiceLoopAddress(u8 arg_voice, u32 arg_address)
{
	...
	SPUwriteRegister(m_PSX_SPU_BASE + arg_voice * 0x10 + 0x0e,
			(u16)(arg_address / 8));
}
		...
		u16 arg_rate, SpuRateMode arg_mode)
{
	...
	if(arg_rate >= 0x80)
	{
		cout << "*** error: wrong attack rate!" << endl;
		return;
	}
		...
		u16 arg_rate, SpuRateMode arg_mode)
{
	...
	if(arg_rate >= 0x80)
	{
		cout << "*** error: wrong sustain rate!" << endl;
		return;
	}
	...
	switch(arg_mode)
	{
		...
		default:
		cout << "*** error: wrong sustain rate mode!" << endl;
		...
		u16 arg_rate, SpuRateMode arg_mode)
{
	...
	if(arg_rate >= 0x20)
	{
		cout << "*** error: wrong release rate!" << endl;
		return;
	}
...
void SoundManager::SetVoiceDecayRate(u8 arg_voice, u16 arg_rate)
{
	...
	if(arg_rate >= 0x10)
	{
		cout << "*** error: wrong decay rate!" << endl;
		return;
	}
...
void SoundManager::SetVoiceSustainLevel(u8 arg_voice, u16 arg_level)
{
	...
	if(arg_level >= 0x10)
	{
		cout << "*** error: wrong sustain level!" << endl;
		return;
	}
...
void SPUwriteDMAMem(unsigned short *pusPSXMem, int iSize)
{
	...
	cout << setfill('0');
	cout << "*** SPUwriteDMAMem() ["
		<< "size: " << dec << iSize * sizeof(unsigned short) << "]" << endl;
#endif
/*
	uint8_t *dma_data = (uint8_t *)pusPSXMem;
	for(int i = 0; i < iSize; ++i)
	{
		if(!(i % 16))
			cout << endl;

		cout << hex << setw(2) << (int)dma_data[i] << ' ';
	}
	cout << endl;
*/
/*
	// dump dma data to file
	ofstream oF;
	oF.open("dmadump.bin", ios_base::binary);
	oF.write((char *)pusPSXMem, iSize * sizeof(unsigned short));
	oF.close();
//	if(iSize < 1000)
		exit(EXIT_SUCCESS);
*/
...
void SPUwriteRegister(unsigned long reg, unsigned short val)
{
#ifdef DEBUG_OUTPUT
	cout << setfill('0');
	cout << "*** SPUwriteRegister() " << "[address 0x"
	   << hex << setw(8) << reg << "] ";
#endif
	...
	if(r >= 0x0c00 && r < 0x0d80)
	{
		...
		switch(r & 0x0f)
		{
			...
			case 10:
#ifdef DEBUG_OUTPUT
				cout << "ADSR Times with Pre-Calcs - ";
#endif
...
unsigned short SPUreadRegister(unsigned long reg)
{
#ifdef DEBUG_OUTPUT
	cout << setfill('0');
	cout << "*** SPUreadRegister(0x" << hex << setw(8) << reg << "), returned ";
#endif
	...
	switch(r)
	{
		case H_SPUctrl:
#ifdef DEBUG_OUTPUT
			cout << "0x" << hex << setw(4) << spuCtrl << endl;
#endif
			return spuCtrl;

		case H_SPUstat:
#ifdef DEBUG_OUTPUT
			cout << "0x" << hex << setw(4) << spuStat << endl;
#endif
			return spuStat;
...
#ifdef DEBUG_OUTPUT
			cout << "0x" << hex
				<< setw(4) << (unsigned short)(spuAddr >> 3) << endl;
#endif
		...
		case H_SPUirqAddr:
#ifdef DEBUG_OUTPUT
			cout << "0x" << hex << setw(4) << spuIrq << endl;
#endif
			return spuIrq;
	}

#ifdef DEBUG_OUTPUT
	cout << "0x" << hex << setw(4) << regArea[(r - 0xc00) >> 1] << endl;
#endif
...
{
//	cout << "*** SPUasync(0x"<< hex << setw(8) << cycle << ")" << endl;
...
void SetupStreams(void)
{
	...
	// alloc xa buffer
	XAStart = (uint32_t *)malloc(44100 * 4);
	XAPlay = XAStart;
	XAFeed = XAStart;
	XAEnd = XAStart + 44100;
...
{
#ifdef DEBUG_OUTPUT
	cout << "*** SPUregisterCDDAVolume()" << endl;
#endif
