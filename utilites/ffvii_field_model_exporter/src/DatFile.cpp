...
{
    LOGGER->Log( "\nDUMP sector 7 for model id " + ToHexString( model_id, 2, '0' ) );

    // get sector 7
    u32 offset = 0x1C + GetU32LE(0x18) - GetU32LE(0x00) + 4 + model_id * 8;

    data.face_id = GetU8(offset + 0x00);
    LOGGER->Log( "face_id: "  + ToHexString( data.face_id, 2, '0' ) );

    data.number_of_bones = GetU8(offset + 0x01);
    LOGGER->Log( "number_of_bones: " + ToHexString( data.number_of_bones, 2, '0' ) );

    data.number_of_parts = GetU8(offset + 0x02);
    LOGGER->Log( "number_of_parts: " + ToHexString( data.number_of_parts, 2, '0' ) );

    data.number_of_animation = GetU8(offset + 0x03);
    LOGGER->Log( "number_of_animation: " + ToHexString( data.number_of_animation, 2, '0' ) );

    data.global_model_id = GetU8(offset + 0x07);
    LOGGER->Log( "global_model_id: %02x\n" + ToHexString( data.global_model_id, 2, '0' ) );
}
