...
{
	public:
		SoundManager(uint16_t arg_frequency, uint8_t arg_output_channels,
		...
		void StopProcessing();
		void Update(uint32_t arg_ms);

		// psx routines
		void PsxInstrumentSetReverbDepth(uint8_t arg_channel,
			uint16_t arg_left, uint16_t arg_right);
		void PsxChannelPlay(uint8_t arg_channel);
		void PsxChannelStop(uint8_t arg_channel);
		void PsxReverbOn(uint8_t arg_channel);
		...
		void SetVoiceVolume(u8 arg_voice,
				s16 arg_volume_left, s16 arg_volume_right);
		void SetVoicePitch(u8 arg_voice, u16 arg_pitch);
		void SetVoiceAttackRate(u8 arg_voice, u16 arg_rate,
				SpuRateMode arg_mode);
		void SetVoiceSustainRate(u8 arg_voice, u16 arg_rate,
				SpuRateMode arg_mode);
		void SetVoiceReleaseRate(u8 arg_voice, u16 arg_rate,
				SpuRateMode arg_mode);
	...
	private:
/*
		typedef void (*SPUinit_t)(void);
		typedef long (*SPUopen_t)(void);
		typedef long (*SPUclose_t)(void);
		typedef void (*SPUasync_t)(unsigned long);
		typedef void (*SPUwriteRegister_t)(unsigned long, unsigned short);
		typedef unsigned short (*SPUreadRegister_t)(unsigned long);
		typedef void (*SPUwriteDMAMem_t)(unsigned short *, int);
		...
		SPUclose_t SPUclose;
		SPUasync_t SPUasync;
		SPUwriteRegister_t SPUwriteRegister;
		SPUreadRegister_t SPUreadRegister;
		SPUwriteDMAMem_t SPUwriteDMAMem;
*/
		bool m_Started;

		static const uint32_t m_REGISTER_BASE = 0x1f801c00;

		bool PsxWriteDMA(uint16_t arg_address, uint8_t *arg_data,
				uint32_t arg_length);
};
